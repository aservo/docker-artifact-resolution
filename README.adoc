...
Each cache entry gets its own subdirectory under the specified cache directory. The name of such a subdirectory is a hash, calculated from attributes that make the artifacts unique. Every cache entry includes a file _cache-entry.txt_ with metadata.
